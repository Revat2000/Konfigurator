...
const alertMsgForMarkingIDF = document.getElementById('alertMsgForMarking');


let finaleMarking = ''; // финальная маркировка
let curType = 'ИД-F'; // модификация ИД
let curTypePreassure = ''; // вид измеряемого давления
let curCase = ''; // тип корпуса ИД
let curEx = ''; // тип взрывозащиты
let curInterface = ''; // тип интерфейса
let curHART = false; // наличие HART - протокола
...
let minTKValue = -40; // минимальное допустимое значение ТК
let maxTKValue = 85; // максимальное допустимое значение ТК
let curMinTK = ''; // минимальное значение диапазона ТК выбранное пользователем
let curMaxTK = ''; // максимальное значение диапазона ТК выбранное пользователем
...
window.onload = function() {DisplayConnectionBox(), additionAccuracyTable()};
...
////////////////////////////////////  Конец вида измеряемого давления  /////////////////////////////////
  ...
  link.addEventListener('click', (e) => {
    ...
    switch (curCase) {
      case 'Тр':
        ...
        NodeChangeClass(['imgN1', 'labelImgN1', 'imgT', 'labelImgT', 'imgTi', 'labelImgTi'], 'remove', 'show')
        NodeVisualise(['exdblabel'], 'none', ['exdb', 'exia'], false);
        break;
      case 'Т':
        NodeChangeClass(['imgN1', 'labelImgN1', 'imgTr', 'labelImgTr', 'imgTi', 'labelImgTi'], 'remove', 'show')
        NodeChangeClass(['imgT', 'labelImgT'], 'add', 'show')
        NodeVisualise(['exdblabel'],'block', ['exdb', 'exia'], false);
        break;
      case 'Ти':
        NodeChangeClass(['imgN1', 'labelImgN1', 'imgTr', 'labelImgTr', 'imgT', 'labelImgT'], 'remove', 'show')
        NodeChangeClass(['imgTi', 'labelImgTi'], 'add', 'show')
        NodeVisualise(['exdblabel'],'block', ['exdb', 'exia'], false);
        break;
      case 'Н1':  
        NodeChangeClass(['imgN1', 'labelImgN1'], 'add', 'show')
        NodeChangeClass(['imgT', 'labelImgT', 'imgTr', 'labelImgTr', 'imgTi', 'labelImgTi'], 'remove', 'show')
        NodeVisualise(['exdblabel'], 'none', ['exdb', 'exia'], false);
        break;
      default:
    ...
    // выбору типов IP
    ipDegree()
    // Отрисовка значений допустимого диапазона ТК
    diapTK();
    // проверка и подсветка изменения маркировки
    chChangeMark = true; // флаг изменнения маркировки
    BGcolorMarkingBlock();
...
////////////////////////////////////  Выходной интерфейс  /////////////////////////////////
const interfaceArr = ['(0-5)мА', '(0-10)мА', '(0-20)мА', '(4-20)мА', '(0.2-5)В', '(0.2-10)В', '(0.4-2)В' ];
createInputs(interfaceArr, 'interface', 'interface', 'curInterface');
////////////////////////////////////  Конец Выходной интерфейс   /////////////////////////////////




////////////////////////////////////  HART  /////////////////////////////////
const checkBoxHART = document.getElementById('checkBoxHART');
checkBoxHART.addEventListener('change', () => {
  if (checkBoxHART.checked) {
    curHART = true;
  } else {
    curHART = false;
  };
...
const accuracyArr = ["±0.075", "±0.1", "±0.15", "±0.2", "±0.25", "±0.5", "±1"];
createInputs(accuracyArr, 'accuracy', 'accuracy', 'curAccuracy');
...
////////////////////////////////////  Термокомпенсация  /////////////////////////////////
// показать/скрыть диапазон компенсации
let chValtKomp = false;
const checkBoxtKomp = document.getElementById('checkBoxtKomp');
checkBoxtKomp.addEventListener('change', () => {
  if (checkBoxtKomp.checked) {
    chValtKomp = true;
    NodeChangeClass(['NOTtKompWrap'], 'remove', 'active');
    NodeChangeClass(['tKompWrap'], 'add', 'active');
    // удаление таблицы
    deleteAllChilds('NOTtKompWrap');
    // Отрисовка значений допустимого диапазона ТК
    diapTK();
  } else if (!checkBoxtKomp.checked) {
    clearTKdiap(); // clear values of TKdiapasones
    chValtKomp = false;
    NodeChangeClass(['tKompWrap'], 'remove', 'active');
    NodeChangeClass(['NOTtKompWrap'], 'add', 'active');
    // отображение таблицы доп погрешности
    additionAccuracyTable();
  };
  // проверка и подсветка изменения маркировки
  chChangeMark = true; // флаг изменнения маркировки
  BGcolorMarkingBlock();
...
// Отрисовка значений допустимого диапазона ТК
function diapTK() {
  const tKompP = document.getElementById('tKompP');
  let labelDiapTK = '';

  if (curCase === 'Ти') {
    labelDiapTK = 'от -40 °C до +70 °C:';
    maxTKValue = 70;
  } else {
    labelDiapTK = 'от -40 °C до +85 °C:';
    maxTKValue = 85;
  };
  clearTKdiap(); // clear values of TKdiapasones
  tKompP.innerText = 'Допустимый диапазон температурной компенсации в пределах ' + labelDiapTK;
  // слушатель для min & max на запрет ввода десятичных знаков
  prohibitionFractional(mintKomp);
  prohibitionFractional(maxtKomp);
};


const mintKomp = document.getElementById('mintKomp');
const maxtKomp = document.getElementById('maxtKomp');

// слушатель для min
mintKomp.addEventListener('input', () => {
  const min = parseInt(mintKomp.value, 10);
  if (min >= minTKValue && min <= maxTKValue) {
    curMinTK = min;
    NodeChangeClass(['alertMsgForTKDiapMin'], 'remove', 'active');
    // проверка и подсветка изменения маркировки
    chChangeMark = true; // флаг изменнения маркировки
    BGcolorMarkingBlock();
  } else if ((min != NaN) && (min < minTKValue || min > maxTKValue)) {
    curMinTK = '';
    NodeChangeClass(['alertMsgForTKDiapMin'], 'add', 'active');
    document.getElementById('alertMsgForTKDiapMin').innerHTML = '<span class="closebtnAlert" onclick="this.parentElement.classList.remove(' 
      + "'active'" + ');">&times;</span><strong>Ошибка!</strong> Значения диапазона температурной компенсации введены неверно.'
  };
  // сравниваем значения диапазона Тк, чтобы max >= min !
  compareTKdiapVal(curMinTK, curMaxTK);
});

// mintKomp.addEventListener('focusout', (e) => {
//   const tVal = mintKomp.value;
//   if (tVal.indexOf('-') > -1) {
//     console.log(tVal);
//   }
//   console.log(tVal.indexOf('-'));
// });

// слушатель для max
maxtKomp.addEventListener('input', () => {
  const max = maxtKomp.value;
  if (max >= minTKValue && max <= maxTKValue) {
    curMaxTK = max;
    NodeChangeClass(['alertMsgForTKDiapMax'], 'remove', 'active');
    // проверка и подсветка изменения маркировки
    chChangeMark = true; // флаг изменнения маркировки
    BGcolorMarkingBlock();
  } else if ((max != NaN) && (max < minTKValue || max > maxTKValue)) {
    curMaxTK = '';
    NodeChangeClass(['alertMsgForTKDiapMax'], 'add', 'active');
    document.getElementById('alertMsgForTKDiapMax').innerHTML = '<span class="closebtnAlert" onclick="this.parentElement.classList.remove(' 
      + "'active'" + ');">&times;</span><strong>Ошибка!</strong> Значения диапазона температурной компенсации введены неверно.'
  };
  // сравниваем значения диапазона Тк, чтобы max >= min !
  compareTKdiapVal(curMinTK, curMaxTK);
...
// сравниваем значения диапазона Тк, чтобы max >= min !
function compareTKdiapVal(min, max) {
  if (max < min && max != '') {
    NodeChangeClass(['alertMsgForTKDiap2'], 'add', 'active');
    document.getElementById('alertMsgForTKDiap2').innerHTML = '<span class="closebtnAlert" onclick="this.parentElement.classList.remove(' 
      + "'active'" + ');">&times;</span><strong>Ошибка!</strong> Верхнее значение диапазона температурной компенсации меньше нижнего.'
  } else {
    NodeChangeClass(['alertMsgForTKDiap2'], 'remove', 'active');
  }
};

// clear values of TKdiapasones
function clearTKdiap() {
  mintKomp.value = '';
  maxtKomp.value = '';
  curMinTK = '';
  curMaxTK = ''; 
  ...
  minTK = '';
  maxTK = '';
  NodeChangeClass(['alertMsgForTKDiapMin', 'alertMsgForTKDiapMax', 'alertMsgForTKDiap2'], 'remove', 'active');
}

// Отрисовка таблицы доп погрешностей при отсутствии компенсации
function additionAccuracyTable() {
  const addAccurArr = {
    "±0.075" : "±0.075", 
    "±0.1" : "±0.1",
    "±0.15" : "±0.15",
  ...
  const placeDiv = document.getElementById('NOTtKompWrap');
  // accuracyArr
   // создание элемента h4
   const h4Elmt = document.createElement('h4');
   h4Elmt.innerHTML = 'При отсутствии температурной компенсации ' + 
                      'предел <font style="color: rgba(235, 61, 61, 0.644);">дополнительной</font> ' + 
                      'погрешности, вызванной изменением температуры окружающего воздуха на каждые 10°С,' +
                      ' не превышает предел основной приведенной погрешности в соответствии с таблицей:';
   placeDiv.appendChild(h4Elmt);

   // создание элемента table
   const tableElmt = document.createElement('table');
   tableElmt.id = 'NOTtKomp_table';
   tableElmt.classList.add('my-table-kvv');
   tableElmt.classList.add('w3-card-4');
   tableElmt.classList.add('accuracyTable');
   placeDiv.appendChild(tableElmt);
   // ссылка на элемент table
   const placeTable = document.getElementById('NOTtKomp_table');

    // создание элемента tr
    const trElmt = document.createElement('tr');
    // создание элемента th
    const thElmt = document.createElement('th');
    thElmt.innerHTML = 'Основная<br>приведенная погрешность';
    trElmt.appendChild(thElmt);
    // создание элемента th
    const thElmt2 = document.createElement('th');
    thElmt2.innerHTML = 'Дополнительная<br>погрешность';
    trElmt.appendChild(thElmt2);
    placeTable.appendChild(trElmt);

    // вставка всех элементов текущего массива в таблицу
    Object.keys(addAccurArr).forEach((key) => {
      const trElmt2 = document.createElement('tr');
      const tdElmt1 = document.createElement('td');
      tdElmt1.innerHTML = key;
      const tdElmt2 = document.createElement('td');
      tdElmt2.innerHTML = addAccurArr[key];

      trElmt2.appendChild(tdElmt1);
      trElmt2.appendChild(tdElmt2);
      placeTable.appendChild(trElmt2);
    });
} 
////////////////////////////////////  Конец Термокомпенсация   /////////////////////////////////
...
function DisplayConnectionBox () {
  ...
  if (curTypePreassure === 'И' || curTypePreassure === 'А') {
    arr4.forEach((elmt1) => {
...
function curKvvArrFunc() {
  ...
  } else if (curEx === 'ExdbIICT6' && (curCase === 'Ти' || curCase === 'Т' || curCase === 'Н1')) {
    // console.log('111');
    kvvForTExd.forEach((el) =>{
      curKvvArr.push(kvvInfoArr[el]);
    })
  } else if ((curEx === 'ExiaIICT6'  || curEx === '') && (curCase === 'Ти' || curCase === 'Т' || curCase === 'Н1')) {
    // console.log('222');
...
markingBTN.addEventListener('click', () =>{
  ...
  if (chValtKomp) {
    ...
    tkompVal = 't(' + minTK + '...' + maxTK + ')-';
  } else {
    tkompVal = '';
  };

  curDiap = '(' + curDiapMin + '...' + curDiapMax + ')';
  let curDiapMain = '-' + curDiap;

  finaleMarking = curType + '-' + curTypePreassure + '-' + curCase + hyp + curEx + '-' + curInterface  + 
                  protokol + curDiapMain + '-('  + curAccuracy + ')-' + 
                  tkompVal + curThread + hyp2 + connection + kvvMarking + gosPoverka;


  // finaleMarking = curType + '-' + curTypePreassure + '-' + curCase + hyp + curEx + '-' + curInterface  + 
  //                 protokol + '-(' + curDiapMin + '...' + curDiapMax + ')-('  + curAccuracy + ')-' + 
  //                 tkompVal + curThread + connection + kvvMarking + gosPoverka;
...
function storeVariables() {
  if (typeof(Storage) !== "undefined") {
    ...
    sessionStorage['tkompVal'] = tkompVal;
    sessionStorage['minTK'] = minTK;
    sessionStorage['maxTK'] = maxTK;
    ...
    sessionStorage['hart'] = hart; 
    sessionStorage['curInterface'] = curInterface;
...
////////////////////////////////////   Общие функции   /////////////////////////////////
// Запрет ввода "," и "."
function prohibitionFractional(inputObgect = Object) {
  inputObgect.addEventListener('keypress', (e) => {
    const num = e.key;
    if (num == '.' || num == ',') {
      e.preventDefault();
      e.stopPropagation();
...
function BGcolorMarkingBlock() {
  ...
  if (chChangeMark) {
    ...
    alertMsgForMarkingIDF.classList.remove('active');
  }
}; 
...
function checkRequiredValues() {
  const varNamesArr = {
    ...
    'curInterface' : {
      'varValue' : curInterface,
      'alertText' : 'Выходной интерфейс',
      'anchorHref' : '#interfaceAnchor'
    },
    'curDiapMin' : {
      'varValue' : curDiapMin,
  ...
  } else if ((curCase === 'Т' || curCase === 'Ти') && (curKVV === '') && (curEx === 'ExdbIICT6' || curIP === 'IP66' || curIP === 'IP68')) {
    alertTextFinale = alertTextFinale + '<a href="#typeKvvAnchor"><li>Тип кабельного ввода</li></a>';
  };
  ...
  // правила заполнения термокомпенсации
  
  if ((chValtKomp === true) && (curMinTK === '' || curMaxTK === '')) {
    alertTextFinale = alertTextFinale + '<a href="#tKompAnchor"><li>Диапазон температурной компенсации</li></a>';
  };
...
//  Функция создания стандартных инпутов и добавления слушателя
function createInputs(curArr = [], putPlace = '', nameInpt = '', lisnerVar = '') { 
  // curArr - массив для распределения в инпуты
  // putPlace - 'id' блока назначения 
  // nameInpt - 'name' блоков инпута
  // lisnerVar - текстовое имя переменной для слушателя текущих инпутов
  const putDiv = document.getElementById(putPlace);
  curArr.forEach((elmt) => {
    // создание элемента Label
    const labelElmt = document.createElement('label');
    labelElmt.innerHTML = elmt;
    labelElmt.classList.add('container');
    labelElmt.htmlFor = elmt;
    labelElmt.id = elmt + '_label'
    putDiv.appendChild(labelElmt);
    // ссылка на созданный элемент label
    const placeLabel = document.getElementById(elmt + '_label');

    // создание элемента input
    const inputElmt = document.createElement('input');
    inputElmt.type = 'radio';
    inputElmt.id = elmt;
    inputElmt.name = nameInpt;
    inputElmt.value = elmt;
    placeLabel.appendChild(inputElmt);

    // слушатель на инпут
    inputElmt.addEventListener('click', () => {
      if (lisnerVar === 'curInterface') {
        curInterface = inputElmt.value;
      } else if (lisnerVar === 'curAccuracy') {
        curAccuracy = inputElmt.value;
      };
      //  проверка и подсветка изменения маркировки
      chChangeMark = true; // флаг изменнения маркировки
      BGcolorMarkingBlock();
